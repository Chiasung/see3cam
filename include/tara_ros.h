...
//	1.2.131.652 is the last firmware version of Tara that doesn't support
//auto exposure.
...
class taraCamera {
 public:
  ...
  // IMU
  void getInclination(double w_x, double w_y, double w_z, double a_x,
                      double a_y, double a_z);
  // getOrientation return the orientation in quaternion format
  void getOrientation(double w_x, double w_y, double w_z, double a_x,
                      double a_y, double a_z);
  int returnValue;
 ...
 private:
  ...
  BOOL DisableIMU();
  BOOL checkFirmware(UINT8 MajorVersion, UINT8 MinorVersion1,
                     UINT16 MinorVersion2,
                     UINT16 MinorVersion3);  // Returns 1 if firmware supports
                                             // auto exposure, else 0;
