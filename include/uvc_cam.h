...
class Cam {
 public:
  ...
  Cam(const char *device, mode_t _mode = MODE_RGB, int _width = 640,
      int _height = 480, int _fps = 30);
  UINT8 MajorVersion;
  UINT8 MinorVersion1;
  ...
  int grabStereo(unsigned char **frame, uint32_t &bytes_used,
                 unsigned char **left_frame, unsigned char **right_frame,
                 unsigned char **concat_frame);
