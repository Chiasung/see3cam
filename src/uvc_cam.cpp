...
static void enumerate_menu(int device_file_h_, struct v4l2_queryctrl &queryctrl,
...
Cam::Cam(const char *_device, mode_t _mode, int _width, int _height, int _fps)
  ...
  while ((ret = ioctl(device_file_h_, VIDIOC_ENUM_FMT, &format_desc)) == 0) {
    printf("pixfmt %d = '%4s' desc = '%s'\n", format_desc.index,
           (char *)&format_desc.pixelformat, format_desc.description);
    ...
    while ((ret = ioctl(device_file_h_, VIDIOC_ENUM_FRAMESIZES, &fsize)) == 0) {
      ...
      if (fsize.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
        printf("  discrete: %ux%u:   ", fsize.discrete.width,
        ...
        /*
          while ((ret = ioctl(device_file_h_, VIDIOC_ENUM_FRAMEINTERVALS,
          &fival)) == 0)
          {
          fival.index++;
        ...
        */
        ...
        printf("  continuous: %ux%u to %ux%u\n", fsize.stepwise.min_width,
               fsize.stepwise.min_height, fsize.stepwise.max_width,
  ...
  try {
    // the commented labels correspond to the controls in guvcview and
    // uvcdynctrl
  ...
  */
  ...
  for (unsigned i = 0; i < NUM_BUFFERS; i++) {
    ...
    if (buffer_.length <= 0) throw std::runtime_error("buffer length is bogus");
    buffer_mem_[i] = mmap(0, buffer_.length, PROT_READ, MAP_SHARED,
                          device_file_h_, buffer_.m.offset);
...
void Cam::showFirmwareVersion() {
  if (true == ReadFirmwareVersion(&MajorVersion, &MinorVersion1, &MinorVersion2,
                                  &MinorVersion3)) {
...
void Cam::enumerate() {
  ...
  if (!d) throw std::runtime_error("couldn't open " + v4l_path);
  ...
  while ((ent = readdir(d)) != NULL) {
    ...
    close(fd);
    string v4l_dev_path =
        v4l_path + string("/") + string(ent->d_name) + string("/device");
    // my kernel is using /sys/class/video4linux/videoN/device/inputX/id
    DIR *d2 = opendir(v4l_dev_path.c_str());
    if (!d2) throw std::runtime_error("couldn't open " + v4l_dev_path);
    string input_dir;
    while ((ent2 = readdir(d2)) != NULL) {
      ...
      DIR *input =
          opendir((v4l_dev_path + string("/") + string(ent2->d_name)).c_str());
      ...
      if (!output_set) input_dir = ent2->d_name;
      break;
    }
    ...
    string vid_fname =
        v4l_dev_path + string("/") + input_dir + string("/id/vendor");
    string pid_fname =
        v4l_dev_path + string("/") + input_dir + string("/id/product");
    string ver_fname =
        v4l_dev_path + string("/") + input_dir + string("/id/version");
    char vid[5], pid[5], ver[5];
    FILE *vid_fp = fopen(vid_fname.c_str(), "r");
    if (!vid_fp) throw std::runtime_error("couldn't open " + vid_fname);
    ...
    if (!pid_fp) throw std::runtime_error("couldn't open " + pid_fname);
    ...
    if (!ver_fp) throw std::runtime_error("couldn't open " + ver_fname);
...
int Cam::grabStereo(unsigned char **frame, uint32_t &bytes_used,
  ...
  *frame = NULL;
  ...
  if (!FD_ISSET(device_file_h_, &rdset)) return -1;
  memset(&buffer_, 0, sizeof(buffer_));
  buffer_.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
  buffer_.memory = V4L2_MEMORY_MMAP;
  ...
  if (mode_ == MODE_RGB) {
    ...
    for (unsigned i = 0; i < width_ * height_ * 2; i += 4) {
      ...
      *prgb++ = sat(pyuv[i + 2] + 1.772f * (pyuv[i + 1] - 128));
      ...
      if ((int)pyuv[i + 2] - (int)pyuv_last[i + 2] >
              motion_threshold_luminance_ ||
          (int)pyuv_last[i + 2] - (int)pyuv[i + 2] >
              motion_threshold_luminance_)
        num_pixels_different++;
       ...
       *prgb++ = sat(pyuv[i+2]+1.772f*(pyuv[i+1]-128));
       *prgb++ =
       *sat(pyuv[i+2]-0.34414f*(pyuv[i+1]-128)-0.71414f*(pyuv[i+3]-128)); prgb++
       *= sat(pyuv[i+2]+1.402f*(pyuv[i+3]-128));
       */
    }
...
int Cam::set_control(uint32_t id, int val) {
  ...
  if (val < queryctrl.minimum || val > queryctrl.maximum) {
    printf(
        "Value of cntrol #%s is out of range, Select value between %d and %d\n",
...
int Cam::GetListofDeviceseCon(void) {
  ...
  udev_list_entry_foreach(dev_list_entry, devices) {
    ...
    //   		        if ((fd = open(v4l2_device, O_RDWR | O_NONBLOCK,
    //   0)) < 0)
    ...
    DeviceInstances->listVidDevices =
        g_renew(VidDevice, DeviceInstances->listVidDevices, num_dev);
    DeviceInstances->listVidDevices[num_dev - 1].device = g_strdup(v4l2_device);
    DeviceInstances->listVidDevices[num_dev - 1].deviceID =
        atoi(DeviceInstances->listVidDevices[num_dev - 1].device + 10);
    DeviceInstances->listVidDevices[num_dev - 1].friendlyname =
        g_strdup((gchar *)v4l2_cap.card);
    DeviceInstances->listVidDevices[num_dev - 1].bus_info =
        g_strdup((gchar *)v4l2_cap.bus_info);
    ...
    DeviceInstances->listVidDevices[num_dev - 1].vendor =
        g_strdup((gchar *)udev_device_get_sysattr_value(dev, "idVendor"));
    DeviceInstances->listVidDevices[num_dev - 1].product =
        g_strdup((gchar *)udev_device_get_sysattr_value(dev, "idProduct"));
