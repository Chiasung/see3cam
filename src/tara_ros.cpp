...
namespace uvc_camera {
     ...
     taraCamera::taraCamera(ros::NodeHandle _comm_nh, ros::NodeHandle _param_nh) :
        ...
        info_mgr_left(_comm_nh, "cameraLeft"), info_mgr_right(_comm_nh, "cameraRight"), cam(0) {
                ...
                ROS_INFO("taraCamera::taraCamera");
                ...
                //======================================================================
                // for IMU
                angular_velocity_stdev_     = 0.0899 * (M_PI / 180.0);  // 0.0899 from measurement // 0.02 deg/s resolution, as per manual
                ...
                imu_publisher_ = node.advertise<ImuMsg>( "imu/data_raw", 5);
                cal_publisher_ = node.advertise<std_msgs::Bool>( "imu/is_calibrated", 5);
                //======================================================================
                ...
                cam = new uvc_cam::Cam(device.c_str(), uvc_cam::Cam::MODE_Y16, width, height, fps);
                     ...
                     ROS_INFO("taraCamera::taraCamera 1");
                     if (cam -> IsStereo == true ){
                        ROS_INFO("taraCamera::taraCamera 2");
                     ...
                     ROS_INFO("taraCamera::taraCamera 3");
        }
...
//             ROS_INFO("sendInfoLeft 1");
        ...
        while (ok) {
            ...
            if (skip_frames == 0 || frames_to_skip == 0) {
               if (img_frame) {
                  ...
                  image->encoding = image_encodings::MONO8; // <<<<<<<<<<<<<< RAW IMAGE FORMAT
     ...
     BOOL taraCamera::LoadCameraMatrix()
     {
         ...
         ROS_INFO("enter BOOL taraCamera::LoadCameraMatrix()");
        ...
        foutLeft.close();
        foutRight.close();
             ROS_INFO("leave BOOL taraCamera::LoadCameraMatrix()");
        return TRUE;
     ...
     void taraCamera::SetIMUConfigDefaultEnable()
     {
        ...
        lIMUConfig.IMU_ODR_CONFIG           = IMU_ODR_238HZ;
        lIMUConfig.ACC_SENSITIVITY_CONFIG   = IMU_ACC_SENS_4G;
     ...
     void taraCamera::IMU_enable()
     {
        ...
        while (ros::ok() ){
           ...
           // **** create and publish imu message;  use http://wiki.ros.org/imu_filter_madgwick  to filter data_raw

           boost::shared_ptr<ImuMsg> imu_msg =
             boost::make_shared<ImuMsg>(imu_msg_);

           imu_msg->header.stamp = ros::Time::now();
           imu_msg->header.frame_id = "taraIMU";

           // set linear acceleration
           imu_msg->linear_acceleration.x = lIMUOutput->accX * G * 1e-3;
           imu_msg->linear_acceleration.y = lIMUOutput->accY * G * 1e-3;
           imu_msg->linear_acceleration.z = lIMUOutput->accZ * G * 1e-3;

           // set angular velocities
           imu_msg->angular_velocity.x = lIMUOutput->gyroX * (M_PI / 180.0);
           imu_msg->angular_velocity.y = lIMUOutput->gyroY * (M_PI / 180.0);
           imu_msg->angular_velocity.z = lIMUOutput->gyroZ * (M_PI / 180.0);

           imu_publisher_.publish(imu_msg);


           // removed  getInclination()!!
